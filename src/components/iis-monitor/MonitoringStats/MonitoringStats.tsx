...
import type { IPMonitoringCheck, DNSStats } from '@/types';
import IPMonitoringStats from './IPMonitoringStats';
import DNSStatsComponent from './DNSStatsComponent';
...
const MonitoringStats: React.FC<MonitoringStatsProps> = ({ ipChecks, dnsStats }) => {
  const hasIPChecks = ipChecks.length > 0;
  const hasDNSStats = Object.keys(dnsStats).length > 0;

  if (!hasIPChecks && !hasDNSStats) {
    return (
      <Card>
        <CardContent className="pt-6">
          <div className="text-center py-8 text-gray-500">
  ...
  return (
    <Tabs defaultValue="realtime" className="space-y-6">
      <TabsList>
        <TabsTrigger value="realtime" disabled={!hasIPChecks}>
          <Activity className="w-4 h-4 mr-2" />
          Tiempo Real
        </TabsTrigger>
        <TabsTrigger value="aggregated" disabled={!hasDNSStats}>
      ...
      <TabsContent value="realtime">
        <IPMonitoringStats ipChecks={ipChecks} />
      </TabsContent>

      <TabsContent value="aggregated">
        <DNSStatsComponent dnsStats={dnsStats} />
      </TabsContent>
    </Tabs>
  );
};
